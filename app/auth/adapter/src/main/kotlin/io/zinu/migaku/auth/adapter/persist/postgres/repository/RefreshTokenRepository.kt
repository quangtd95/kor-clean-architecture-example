...
object RefreshTokenRepository : RefreshTokenPort {
    override fun newRefreshToken(userId: String, token: String, expiredTime: LocalDateTime) =
        RefreshToken.new {
    ...
    override fun verifyToken(token: String) =
        RefreshToken
            .find { RefreshTokens.token eq token and (revoked eq false) }
            .firstOrNull() != null

    override fun revokeAllTokens(userId: String) {
        RefreshToken
            .find { RefreshTokens.userId eq UUID.fromString(userId) and (revoked eq false) }
            .forEach { it.revoked = true }
    }

    override fun deleteToken(token: String) {
        RefreshToken
            .find { RefreshTokens.token eq token }
            .firstOrNull()
            ?.delete()
    }
}